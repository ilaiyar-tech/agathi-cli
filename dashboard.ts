...
const screen = blessed.screen({ smartCSR: true, title: 'Agathi AI Lab' });
const grid = new contrib.grid({ rows: 12, cols: 12, screen: screen });

const chatView = grid.set(0, 0, 9, 9, contrib.log, { label: 'Agathi AI Lab (Local-First)', style: { fg: 'white' }, tags: false });
...
const input = grid.set(9, 0, 3, 12, blessed.textbox, {
    label: 'Prompt Agathi (Local Inference)',
    inputOnFocus: true,
    border: { type: 'line' },
...
input.on('submit', (value) => {
    ...
    if (prompt !== "") {
        ...
        proc.stdout.on('data', (data) => {
            chatView.log(`Agathi: ${data.toString().trim()}`);
            screen.render();
        });
